		...
		if(Loader.isModLoaded("Railcraft"))
		{
			if(OreDictionary.getOres("ingotSteel").size() != 0)
			{
				if(ARGetSmelting("SteelTool",true))
				{
					...
					{
						FurnaceRecipes.smelting().addSmelting(GameRegistry.findItem("Railcraft","railcraft.tool.steel.axe"),new ItemStack(OreDictionary.getOres("ingotSteel").get(0).getItem(),3,OreDictionary.getOres("ingotSteel").get(0).getItemDamage()),0.0f);
						if(ARGetAnother("ConsoleOut",true))
						{
		...
		*/
		...
		if((OreDictionary.getOres("oreTofu").size() > 0) && (OreDictionary.getOres("tofuGem").size() > 0))
		{
			...
			GameRegistry.addSmelting(
				OreDictionary.getOres("oreTofu").get(0).getItem(),
				new ItemStack(OreDictionary.getOres("tofuGem").get(0).getItem(),2,OreDictionary.getOres("tofuGem").get(0).getItemDamage()),
				0.0f);
		}
		try
			...
			GameRegistry.addRecipe(
				new ShapedOreRecipe(
					...
					new Object[]{"XYX","XZX","XYX",
						...
						Character.valueOf('Y'),Blocks.stone_slab,
